...
export default function Creative3DStage({ playClick, playHover }: Creative3DStageProps) {
  ...
  return (
    <div 
      ...
      className="rounded-2xl bg-[#fdfcf9] shadow-2xl overflow-hidden flex flex-col h-full min-h-[480px] lg:min-h-[500px] relative text-left p-0"
    >
      {/* Professional Player Top Telemetry Header */}
      <div className="p-3.5 sm:p-4 border-b border-stone-200/60 bg-white/80 flex flex-col gap-2 z-10 backdrop-blur-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Layers2 size={14} className="text-violet-600 animate-spin-slow" />
            <span className="font-mono text-[10px] font-black uppercase tracking-widest text-stone-600">
        ...
        <div className="grid grid-cols-5 gap-1 bg-stone-100/50 p-0.5 rounded-xl border border-stone-200/60">
            ...
            return (
              <button
                ...
                className={`py-1.5 text-[8px] sm:text-[9.5px] font-mono font-bold uppercase rounded-lg transition-all duration-300 cursor-pointer ${
                  isActive 
                    ? 'bg-white text-violet-700 shadow-sm border border-stone-200/80 scale-[1.01]' 
      ...
      <div 
        ref={stageRef}
        className="flex-1 relative overflow-hidden flex flex-col justify-between p-4 sm:p-5 select-none bg-stone-50 min-h-[290px] lg:min-h-[300px]"
        style={{
      ...
      >
        ...
        <div className="w-full flex justify-between items-start z-10 pointer-events-none">
          <div className="space-y-1">
            ...
            <h3 className="font-display font-black text-stone-900 text-lg sm:text-2xl tracking-tight leading-none uppercase">
              {activePage.tagline}
            </h3>
          </div>
          <span className="font-mono text-[8px] text-stone-500 uppercase border border-stone-200/80 px-2 py-0.5 rounded-md bg-stone-50/90">
          ...
          <AnimatePresence mode="wait">
            ...
            {activePage.id === 'hero' && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                className="w-full max-w-[340px] bg-white border border-stone-200 rounded-2xl p-4 flex flex-col space-y-3 shadow-2xl relative"
              >
                {/* Hero Header Mockup */}
                <div className="flex justify-between items-center border-b border-stone-200 pb-2">
                  <span className="font-mono text-[8px] font-black text-stone-900">DEVIL LABS</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-left pt-1">
                  ...
                  <div 
                    className={`p-2.5 rounded-xl border transition-all duration-300 ${
                      heroHoveredCard === 0 
                        ? 'bg-stone-50 border-violet-300 shadow-lg scale-[1.03]' 
                        : 'bg-stone-100 border-stone-200'
                    }`}
                  >
                    <Sparkles size={11} className="text-violet-600 mb-1" />
                    <h4 className="font-mono text-[8px] font-black text-stone-900 leading-none uppercase">3D Motion Lab</h4>
                    <p className="text-[6px] text-stone-500 mt-1 leading-normal">Liquid interactive spatial grids.</p>
                  </div>
                  ...
                  <div 
                    className={`p-2.5 rounded-xl border transition-all duration-300 ${
                      heroHoveredCard === 1 
                        ? 'bg-stone-50 border-fuchsia-300 shadow-lg scale-[1.03]' 
                        : 'bg-stone-100 border-stone-200'
                    }`}
                  >
                    <Activity size={11} className="text-fuchsia-600 mb-1" />
                    <h4 className="font-mono text-[8px] font-black text-stone-900 leading-none uppercase">Pure Systems</h4>
                    <p className="text-[6px] text-stone-500 mt-1 leading-normal">Optimized sub-millisecond loads.</p>
                  </div>

                </div>

                {/* Automated micro feedback counter */}
                <div className="bg-white p-2 rounded-xl border border-stone-200 flex justify-between items-center text-left">
                  <div>
            ...
            {activePage.id === 'services' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -15 }}
                className="w-full max-w-[340px] bg-white/80 border border-stone-200 rounded-2xl p-4 flex flex-col space-y-3.5"
              >
                {/* Simulated service tree / CDN topology graph */}
                <div className="h-28 bg-stone-50 rounded-xl border border-stone-200 relative p-3 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-around py-3 z-10 relative">
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full border flex items-center justify-center transition-all ${activeNode === 'cdn' ? 'bg-emerald-100 border-emerald-300 text-emerald-600 scale-105' : 'bg-white border-stone-200 text-stone-500'}`}>
                        <Globe size={11} />
                      </div>
                      <span className="text-[5px] font-mono mt-1 text-stone-500">EDGE_CDN</span>
                    </div>

                    <div className="h-0.5 w-8 bg-stone-200 relative">
                      {activeNode === 'db' && <div className="absolute inset-y-0 left-0 w-2 bg-emerald-500 rounded-full animate-ping" />}
                    </div>

                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full border flex items-center justify-center transition-all ${activeNode === 'db' ? 'bg-emerald-100 border-emerald-300 text-emerald-600 scale-105' : 'bg-white border-stone-200 text-stone-500'}`}>
                        <Database size={11} />
                      </div>
                      <span className="text-[5px] font-mono mt-1 text-stone-500">LEDGER_DB</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full border flex items-center justify-center transition-all ${activeNode === 'api' ? 'bg-emerald-100 border-emerald-300 text-emerald-600 scale-105' : 'bg-white border-stone-200 text-stone-500'}`}>
                        <Cpu size={11} />
                      </div>
                      <span className="text-[5px] font-mono mt-1 text-stone-500">MICRO_CORE</span>
                    </div>
                  </div>
                ...
                <div className="bg-white border border-stone-200 p-2.5 rounded-xl text-left">
                  <p className="font-mono text-[7.5px] text-stone-600 leading-normal">
            ...
            {activePage.id === 'pricing' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -15 }}
                className="w-full max-w-[300px] bg-white border border-stone-200 rounded-2xl p-3.5 sm:p-4 flex flex-col space-y-2.5 text-left shadow-2xl relative overflow-hidden"
              >
                ...
                <div className="space-y-0.5">
                  <div className="flex items-center space-x-1 text-stone-500 font-mono text-[8px] uppercase tracking-wider font-black">
                    <span className="text-violet-600">$</span>
                    <span>ESTIMATED INVESTMENT</span>
                  </div>
                  <div className="text-2xl sm:text-3xl font-display font-black text-stone-900 tracking-tighter">
                ...
                <div className="space-y-0.5">
                  <div className="flex items-center space-x-1.5 text-stone-500 font-mono text-[8px] uppercase tracking-wider font-black">
                    <Clock size={9} className="text-violet-600" />
                    <span>PROJECTED TIMELINE</span>
                  </div>
                  <div className="text-base font-mono text-stone-700 font-bold">
                    7-12 Weeks
                  </div>
                </div>
                ...
                <div className="space-y-1.5 pt-1 border-t border-stone-200">
                  ...
                  <ul className="space-y-1 text-[9px] text-stone-600 font-sans pl-1">
            ...
            {activePage.id === 'insights' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -15 }}
                className="w-full max-w-[340px] bg-white/80 border border-stone-200 rounded-2xl p-4 flex flex-col space-y-3"
              >
                <div className="flex justify-between items-center">
                  <span className="font-mono text-[7px] text-stone-500 uppercase tracking-widest font-black block">SYSTEM_TELEMETRY: RUNNING</span>
                  <div className="flex space-x-1 bg-stone-100 p-0.5 rounded border border-stone-200">
                    <span className={`text-[5px] font-mono px-1 py-0.5 rounded ${insightsActiveMetric === 'latency' ? 'bg-cyan-100 text-cyan-600 font-bold' : 'text-stone-500'}`}>LATENCY</span>
                    <span className={`text-[5px] font-mono px-1 py-0.5 rounded ${insightsActiveMetric === 'bandwidth' ? 'bg-cyan-100 text-cyan-600 font-bold' : 'text-stone-500'}`}>BW</span>
                  </div>
                </div>

                {/* SVG Live Render Graph */}
                <div className="h-24 bg-white rounded-xl border border-stone-200 p-2 relative flex items-end">
                ...
                <div className="grid grid-cols-2 gap-2 text-left">
                  <div className="bg-white p-2 rounded-xl border border-stone-200">
                    ...
                    <span className="font-mono text-[9px] text-stone-900 font-bold block">99.85% SPEED</span>
                  </div>
                  <div className="bg-white p-2 rounded-xl border border-stone-200">
                    <span className="font-mono text-[5px] text-stone-500 block uppercase">DB READ RATIO</span>
                    <span className="font-mono text-[9px] text-stone-900 font-bold block">1.1ms QUERIES</span>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {activePage.id === 'docking' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -15 }}
                className="w-full max-w-[320px] bg-white border border-stone-200 rounded-2xl p-4 flex flex-col space-y-3.5 text-left shadow-2xl"
              >
                <div className="flex justify-between items-center pb-1 border-b border-stone-200">
                  <span className="font-mono text-[7px] text-stone-500 uppercase tracking-widest font-black">✦ CUSTOMER DOCKING SYSTEM</span>
                ...
                <div className="space-y-2 text-left">
                  <div className="space-y-1">
                    ...
                    <div className="bg-stone-50 border border-stone-200 rounded-lg px-2 py-1 text-[8px] text-stone-700 font-mono">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="h-8 bg-stone-50 border border-stone-200 rounded-lg p-2 flex items-center justify-between">
        ...
        <div className="w-full z-10 bg-white/90 border border-stone-200/60 px-4 py-3 rounded-2xl flex flex-col space-y-1 backdrop-blur-xl shadow-sm">
          <div className="flex items-center justify-between">
            <span className="font-mono text-[7px] text-stone-500 uppercase tracking-widest font-black leading-none">
              ✦ PROCESS SIMULATION NOTES
            </span>
            <span className="text-[6px] font-mono text-stone-400 leading-none">AUTOMATED CURSOR LOGIC ACTIVE</span>
          </div>
          <p className="font-sans text-[10px] text-stone-700 leading-relaxed font-medium">
        ...
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 pointer-events-none select-none z-10">
          <span className="font-mono text-[8px] text-stone-400 uppercase tracking-widest animate-pulse font-bold bg-white/60 px-2 py-1 rounded-full backdrop-blur-sm">
      ...
      <div className="p-4 border-t border-stone-200/60 bg-white/90 flex flex-col gap-3 z-10 text-stone-600 backdrop-blur-xl">
        
        {/* Seekable Playback Timer Track */}
        <div className="w-full flex items-center space-x-3">
          <span className="font-mono text-[9px] text-stone-500 select-none font-medium">
            {formatTime(currentElapsedSeconds)}
          </span>
          
          <div className="flex-1 h-2 bg-stone-100 border border-stone-200/60 rounded-full relative overflow-hidden shadow-inner">
            <div 
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <button 
              ...
              className="p-1.5 hover:bg-stone-100 rounded-lg text-stone-500 hover:text-violet-700 transition-all cursor-pointer"
          ...
          <div className="flex items-center space-x-1 bg-stone-50 px-2 py-1 rounded-full border border-stone-200">
