...
export default function BlurredImage({
  ...
  return (
    <div className={`relative overflow-hidden bg-[#07060d] select-none max-w-full ${className}`}>
      {/* Phase 0: SVG base brand gradient fallback (Loads instantly with 0ms delay) */}
      <img
        src={svgPlaceholder}
        alt=""
        aria-hidden="true"
        className="absolute inset-0 w-full h-full object-cover max-w-full pointer-events-none"
      />
      ...
      {lowResUrl && (
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover max-w-full scale-115 filter blur-xl transition-all duration-500 ${
      ...
      <img
        ...
        className={`w-full h-full object-cover max-w-full absolute inset-0 z-10 transition-all duration-700 ease-out ${
