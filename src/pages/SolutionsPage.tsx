...
export default function SolutionsPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return (
    <div id="solutions-page-root" className="pt-12 sm:pt-16 lg:pt-20 pb-24 px-4 md:px-8 max-w-7xl mx-auto text-stone-100">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-4.5 rounded-2xl border text-left transition-all cursor-pointer ${
                  isActive 
                    ? 'bg-[#050505] border-white/10 text-stone-100 shadow-md' 
                    : 'bg-[#111]/60 border-white/10/50 hover:bg-[#111] text-stone-300 hover:text-white'
                }`}
              >
                <div className="flex items-center space-x-3.5">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors ${
                    isActive ? 'bg-[#0a0a0a]/15 text-violet-400' : 'bg-white/10 text-stone-400'
                  }`}>
                    <Icon size={16} />
                  </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
                <div>
                  <span className="text-violet-400 font-sans text-[10px] uppercase tracking-widest font-extrabold block mb-1">
                ...
                <div className="w-12 h-12 rounded-full bg-violet-950/60 text-violet-400 flex items-center justify-center border border-violet-500/30 flex-shrink-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 pt-2">
                <div className="space-y-2">
                  <div className="flex items-center space-x-2 text-[10px] font-sans uppercase tracking-widest text-red-400 font-extrabold">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2 text-[10px] font-sans uppercase tracking-widest text-emerald-400 font-extrabold">
              ...
              <div className="bg-[#111116] border border-white/10/30 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 text-center sm:text-left">
                {selected.metrics.map((metric, idx) => (
                  <div key={idx} className="space-y-1 border-white/10 sm:border-r last:border-r-0 pr-4">
                    <div className="flex items-center justify-center sm:justify-start space-x-1 text-violet-400">
                      <TrendingUp size={14} />
                      <span className="text-[9px] font-sans uppercase tracking-widest font-extrabold">KEY OUTCOME</span>
                    </div>
                    <p className="text-stone-100 font-display font-black text-lg uppercase tracking-tight">{metric}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6.5">
                  {selected.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start space-x-2.5 text-stone-300 text-xs font-sans">
              ...
              <div className="space-y-2.5 pt-4 border-t border-white/10/30">
                <span className="text-[10px] font-sans uppercase tracking-widest text-stone-400 font-extrabold block">
                  ✦ TECHNOLOGIES &amp; PLATFORMS
                </span>
                <div className="flex flex-wrap gap-2">
                  {selected.techStack.map((tech) => (
                    <span key={tech} className="bg-[#111] text-stone-650 px-3 py-1.5 rounded-full text-[10px] font-sans font-bold border border-stone-250/20 uppercase">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="flex items-center justify-center space-x-2 px-6 py-3 rounded-full bg-[#050505] hover:bg-[#0a0a0a] border border-white/10 text-stone-100 text-xs font-sans font-bold uppercase tracking-widest shadow-md transition-all cursor-pointer"
                >
      ...
      <section className="bg-[#050505] border border-white/10 text-stone-100 rounded-3xl p-8 sm:p-12 relative overflow-hidden text-left shadow-xl">
        ...
        <div className="max-w-2xl space-y-6 relative z-10">
          ...
          <button
            ...
            className="flex items-center justify-center space-x-2 px-6 py-3.5 bg-gradient-to-r from-violet-600 to-rose-500 rounded-full text-xs font-sans font-bold uppercase tracking-widest transition-all cursor-pointer text-[#0a0a0e] shadow-md hover:scale-102"
          >
