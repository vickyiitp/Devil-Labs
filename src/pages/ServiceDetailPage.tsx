...
export default function ServiceDetailPage({
  ...
  return (
    <div className="pt-28 pb-24 px-4 md:px-8 max-w-7xl mx-auto text-stone-100">
      <div className="mb-16">
        <button
          onClick={() => navigate("/services")}
          className="text-stone-400 hover:text-white font-sans text-xs font-bold tracking-widest uppercase mb-8 flex items-center space-x-2 transition-colors clay-button px-4 py-2 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-20 max-w-sm md:max-w-2xl mx-auto">
        {data.features.map((feature: any, idx: number) => {
          ...
          return (
            ...
            >
              <div className="absolute top-4 right-4 flex items-center space-x-2 bg-[#111]/80 px-2.5 py-1 rounded-full border border-white/10">
                <span
              ...
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="font-display font-extrabold text-xl text-stone-100 uppercase tracking-tight">
                  {feature.title}
                </h3>
              </div>
              <p className="text-stone-300 text-sm leading-relaxed pl-7 pr-12 font-sans font-light">
                {feature.desc}
              </p>
            </motion.div>
          );
      ...
      {data.plans && data.plans.length > 0 && (
        <div className="mb-20">
          <div className="mb-10 text-center">
            <span className="text-violet-400 font-mono text-xs uppercase tracking-widest font-bold mb-3 block">
              TRANSPARENT PRICING
            </span>
            <h2 className="font-display font-extrabold text-3xl text-stone-100 tracking-tighter uppercase">
              Service Plans
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
            {data.plans.map((plan: any, idx: number) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: idx * 0.1 }}
                className={`p-6 md:p-8 rounded-[32px] clay-card relative flex flex-col justify-between text-left ${ plan.highlight ? 'border-violet-500/60 bg-violet-950/40 shadow-xl' : '' }`}
              >
                {plan.highlight && (
                ...
                <div>
                  <h3 className="font-display font-extrabold text-xl text-stone-100 uppercase tracking-tight mb-2">
                    {plan.name}
                  </h3>
                  <div className="text-3xl font-display font-black text-stone-100 mb-4">
                    {(() => {
                  ...
                  <p className="text-stone-400 text-xs mb-6 leading-relaxed font-light">
                    {plan.description}
                  </p>
                  <div className="space-y-3 mb-8 border-t border-white/10 pt-4">
                    {plan.features.map((feat: string, i: number) => (
                      <div key={i} className="flex items-start space-x-3 text-xs">
                        ...
                        <span className="text-stone-300 font-sans font-light">{feat}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="mb-20">
          ...
          <div className="space-y-4 max-w-4xl mx-auto">
            {data.faqs.map((faq: any, idx: number) => (
              <details
                key={idx}
                className="group p-6 md:p-8 cursor-pointer rounded-[24px] clay-card hover:scale-[1.01] transition-all duration-300 text-left"
              >
                <summary className="font-display font-extrabold text-lg text-stone-100 list-none flex justify-between items-center outline-none select-none">
                  <span>{faq.q}</span>
                  <span
                ...
                <div
                  className="mt-4 text-stone-300 text-sm leading-relaxed pl-1 font-sans font-light border-t border-white/10 pt-4 text-left"
                  role="region"
                  aria-label={faq.q}
                >
                  {faq.a}
      ...
      <div className="p-6 md:p-8 flex flex-col items-center text-center space-y-6 max-w-4xl mx-auto relative overflow-hidden clay-card bg-gradient-to-br from-[#0a0a0e] to-[#0d0d12]">
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-64 h-32 bg-violet-600/10 blur-[100px] pointer-events-none" />

        <span className="text-violet-400 font-sans text-xs uppercase tracking-widest font-extrabold relative z-10">
          ✦ GET STARTED WITH THIS SERVICE
        </span>
        <h2 className="font-display font-extrabold text-3xl sm:text-5xl text-stone-100 uppercase tracking-tighter leading-tight relative z-10">
          Ready to launch <br />
        ...
        <p className="text-stone-400 text-sm max-w-md leading-relaxed relative z-10 font-sans font-light">
